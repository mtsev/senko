...
class Keywords(Cog):
    ...
    async def _send_notification(self, user_id: str, message: Message, quote: str, word: str) -> None:
        ...
        try:
            ...
            if err.code == 50007:
                log.debug(f"Couldn't DM notify user {user.name}")
    ...
    async def _send(self, ctx: Context, words: list) -> None:
        ...
        try:
            ...
            if err.code == 50007:
                ...
                await ctx.send(f"<@!{ctx.author.id}>, I couldn't send you a DM. Please go to 'Privacy Settings' for this server and allow direct messages from server members.", delete_after=5.0)
            else:
                log.console(err)
